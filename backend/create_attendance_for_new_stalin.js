...
async function createSampleAttendanceForStalin() {
  try {
    console.log("🔧 Creating sample attendance data for stalin j (ID 90)...");

    const employeeId = 90; // stalin j's new user ID
    ...
    verification.rows.forEach((row) => {
      console.log(
        `  ${row.date}: ${row.status} (${row.clock_in_time || "N/A"} - ${
...
createSampleAttendanceForStalin();
